...
//Help from Jake Davis. Approach and algorithm
  ...
  def inc(x: Double) = x + 1                      //> inc: (x: Double)Double
	...
	def firstDouble = selfIter(double _, 1) //> oneIterDouble: => Double => Double
	def secondDouble = selfIter(double _, 2) //> twoIterDouble: => Double => Double
	def thirdDouble = selfIter(double _, 3)
                                                  //> threeIterDouble: => Double => Double
  ...
  def functionality(f: Double => Double) = {
		...
		df _
	}                                         //> driv: (f: Double => Double)Double => Double
  ...
  math.pow(6, 2)                                  //> res10: Double = 1679616.0
  nthRoot(1679616.0, 2)                          //> res11: Double = 6.0
}
